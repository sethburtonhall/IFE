	...
	*/
	class advanced_code_editor{
		...
		public function download_theme(){
			...
			//Get the directory to zip
			$directory = $themes[$theme]->theme_root . '/';
			...
			$fsize = filesize($file);
			header($_SERVER['SERVER_PROTOCOL'].' 200 OK');
			header("Content-Disposition: attachment; filename=\"".strtolower($theme) . '.zip'."\"");
	    	header('Content-Description: File Transfer');
			header('Content-Type: application/octet-stream');
			header('Content-Transfer-Encoding: binary');
			header('Expires: 0');
		...
		public function _create_meta_table($table=null) {
		    ...
		    $query = $wpdb->prepare("CREATE TABLE `{$wpdb->prefix}{$table}` (
